...
MCU_RGB* Read_File(data_frame data, uint16_t number)
{
    ...
    uint32_t position_debut = data.header * sizeof(uint8_t) + (debut_ligne - 1) * data.nb_colonne * sizeof(uint8_t) * taille + (debut_colonne - 1) * sizeof(uint8_t) * taille;
...
//     mcu = Read_File(filename, data, 74);
//     for(uint8_t i = 0; i < MCU_TAILLE; i++)
//     {
//         for(uint8_t j = 0; j < MCU_TAILLE; j++)
//         {
